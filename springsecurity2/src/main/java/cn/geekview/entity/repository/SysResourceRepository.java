...
public interface SysResourceRepository extends JpaRepository<SysResource,Integer> {
     ...
     *
     */
    @Query(value = "SELECT resource_string FROM sys_resource  WHERE resource_id = ( SELECT resource_id FROM sys_resource_role  WHERE role_id = ( SELECT  id  FROM sys_role  WHERE username = ?1))",nativeQuery = true)
